...
# IMPORTANT: utilization is temporarily disabled (we return None for util),
# so strict borrow confirmation = CTB + Availability required.
...
# ============================================================
# ORTEX DATA (FIXED ENDPOINTS + BACKOFF)
# ============================================================
def ortex_get(url: str, *, max_retries: int = 2) -> dict:
    """
    ORTEX can throttle (429). We'll do a small backoff and retry.
    Also prints 400+ errors (like malformed URL) to your run log.
    """
    for attempt in range(max_retries + 1):
        r = SESSION.get(url, headers={"Ortex-Api-Key": ORTEX_KEY}, timeout=30)

        if r.status_code == 429:
            # Try to parse "Expected available in XX seconds"
            try:
                j = r.json()
            except Exception:
                j = {}
            ...
            if attempt >= max_retries:
                print(f"[ORTEX THROTTLED] {url} — giving up after {max_retries+1} tries.")
                return {}

            print(f"[ORTEX THROTTLED] waiting {min(wait_s,60)}s then retrying ({attempt+1}/{max_retries})…")
...
def ortex_ctb_latest(ticker: str) -> float | None:
    # Valid endpoints we’ve seen work on many accounts:
    endpoints = [
...
def ortex_availability_latest(ticker: str) -> float | None:
    # Some accounts support /availability, others may differ.
...
def ortex_utilization_latest(ticker: str) -> float | None:
    """
    Utilization endpoints you were calling were invalid and causing 400s.
    We'll re-add utilization once we confirm your exact ORTEX endpoint for it.
    For now: return None so scans don't break.
    """
    return None
...
def data_quality_penalty(feat: dict) -> tuple[bool, str]:
    ...
    # utilization is intentionally missing right now
...
def is_true_squeeze_strict(feat: dict) -> bool:
    ...
    # STRICT borrow confirmation = CTB + Availability required (utilization disabled for now)
...
<div class="card">
  <h2>TRUE SQUEEZE</h2>
  <small>Strict squeeze: requires CTB + Availability present (utilization temporarily disabled).</small>
  {table_html(sq)}
</div>
    ...
    for pmin, pmax in PRICE_TIERS:
        ...
        for t in candidates:
            try:
                ...
                util = ortex_utilization_latest(t)     # currently None
                ...
                feat = {
                    ...
                    **si_feat,
                    "ctb": ctb,
                    "util": util,          # ✅ IMPORTANT FIX (so Util shows + scoring uses correct key)
                    "avail": avail,
...
# MODES
# ============================================================
def run_one_shot(force: bool = False, run_id: str | None = None):
    if run_id:
...
def main():
    ...
    if args.mode == "oneshot":
        run_one_shot(force=args.force, run_id=args.run_id)
    else:
        run_loop_forever()

...
# ============================================================
# ENGINE V2.4 — MODE (AUTO/DAY/NIGHT) + ORTEX TOGGLE (AUTO/ON/OFF)
# Streams ONLY final top rows (<=5) to UI
# ============================================================
...
SESSION.headers.update({"User-Agent": "precipice-analytica/engine-v2.4"})
...
def ortex_get(url: str, log_fn=None) -> dict | None:
    ...
    if r.status_code in (404, 429) or r.status_code >= 400:
        if log_fn and r.status_code not in (404,):
            log_fn(f"[ORTEX {r.status_code}] {url}")
        return None
...
def confidence_1_to_10(prob: float, ortex_on: bool, has_borrow: bool) -> int:
    p = prob * (1.0 if ortex_on else 0.86)
...
# ---------------- ortex toggle behavior ----------------
def resolve_ortex_on(mode: str, ortex_setting: str, dt: datetime) -> tuple[bool, str]:
    """
    Returns (ortex_on, ortex_label_for_ui)
    ortex_setting: auto|on|off (requested)
    """
    ortex_setting = (ortex_setting or "auto").strip().lower()
    if ortex_setting not in ("auto", "on", "off"):
        ortex_setting = "auto"

    has_key = bool(ORTEX_KEY)

    if ortex_setting == "off":
        return False, "OFF"

    if ortex_setting == "on":
        # force on only if key exists AND mode is day (keeps your cost control)
        if (mode == "day") and has_key and ortex_allowed_now(dt):
            return True, "ON"
        # if user forces ON but hours/key/mode prevent it, show why in logs later
        return False, "OFF"

    # auto
    ortex_on = (mode == "day") and has_key and ortex_allowed_now(dt)
    return ortex_on, ("ON" if ortex_on else "OFF")
...
def run_scan(log_fn=None, row_fn=None, mode: str = "day", ortex: str = "auto") -> str | None:
    ...
    if log_fn:
        ...
        log_fn(f"ORTEX requested: {(ortex or 'auto').upper()}  | effective: {ortex_label}")
        if (ortex or "auto").lower().strip() == "on" and not ortex_on:
            reason = []
            if not ORTEX_KEY:
                reason.append("missing ORTEX_API_KEY")
            if mode != "day":
                reason.append("mode is not DAY")
            if not ortex_allowed_now(dt):
                reason.append("outside 7:30AM–4:00PM CT")
            if reason:
                log_fn("ORTEX not enabled (" + ", ".join(reason) + ").")
    ...
    # PASS 1 (Polygon-only)
    for idx, (_proxy, ticker, snap_meta) in enumerate(deep_list, start=1):
    ...
    # Choose ORTEX finalists (best candidates)
    finalists = []
    ...
    for r in finalists:
        ...
        try:
            ...
            base_score, prob, pressure = compute_scores(feat)
            has_borrow = (ctb is not None and avail is not None)
    ...
    # FINAL DISPLAY LISTS
    squeezes_true = [x for x in analyzed.values() if x.get("bucket") == "SQUEEZE"]

    momentum = [
        x for x in analyzed.values()
        if x.get("bucket") != "SQUEEZE"
        and (x.get("move_pct") is not None)
        and (x["move_pct"] > 0)  # gainers only
    ]
    momentum = sorted(momentum, key=lambda x: (x.get("confidence", 0), x.get("base_score", 0)), reverse=True)[:TOP_N_PER_BUCKET]

    if not squeezes_true:
        watch_sorted = sorted(
            analyzed.values(),
            key=lambda x: (x.get("pressure_score", 0), x.get("base_score", 0), (x.get("dollar_vol") or 0)),
            reverse=True
        )
        squeezes = []
        for x in watch_sorted:
            rr = dict(x)
            rr["bucket"] = "SQUEEZE"
        ...
        squeezes = sorted(squeezes_true, key=lambda x: (x.get("confidence", 0), x.get("base_score", 0)), reverse=True)[:TOP_N_PER_BUCKET]

    # Stream ONLY the final top rows to the website
    if row_fn:
